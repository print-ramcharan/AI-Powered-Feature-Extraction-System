...
- **IMMEDIATE BRANCHING:** Immediately upon cloning this repository, DO NOT work on the `main` branch. You must explicitly create and switch to a new isolated feature branch for your specific task (e.g., `git checkout -b feature/sneha-roads-model`).
- **APPROVAL BOUNDARIES:** If Nikitha, Sanjay, or Sneha want to modify a core pipeline script owned by Ram (like `inference_pipeline.py`), their agent MUST commit the changes to their feature branch, and wait for Ram's agent to explicitly review and merge it. Agents must not push directly to `main` to bypass this.
